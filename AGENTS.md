...
- When asked to draft or write release notes for a tag (this repo or `naftiko/fleet`, which also covers `warden`/`skipper`/`crafter`), load and follow the `release-notes-drafting` skill via the `agents-shared` capability — see `.github/instructions/agents-shared.instructions.md` for the discovery and sync procedure. It resolves the previous tag, the boat codename, and every fact from the real commit range itself — it does not need the previous release link or the boat name supplied manually
