...
# Bearer port: static token — write into secrets.yaml before readiness check
STATIC_TOKEN="${MCP_SERVER_TOKEN}"
sed -i "s|mcp-server-token: .*|mcp-server-token: $STATIC_TOKEN|" "$SECRETS_DIR/secrets.yaml"
