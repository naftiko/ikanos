...
> **Native HTTP-serve was broken until [#594](https://github.com/naftiko/ikanos/issues/594) (now
> fixed) — and the #581 smoke does not catch it.** The nightly native smoke only **TCP-probes** the
> exposed REST port (`/dev/tcp/127.0.0.1/$REST_PORT` in the `test-binary` job — it *deliberately* does
> not run an HTTP client), so it proves the socket *binds* but never that a request is *served*. In
> fact the native binary **bound the port yet hung on every HTTP request** (the JVM served fine)
> because conditional Jetty/Restlet reflection classes were missing from `reflect-config.json`. The
> fix shipped as **[#594](https://github.com/naftiko/ikanos/issues/594) (CLOSED) via PR
> [#596](https://github.com/naftiko/ikanos/pull/596), MERGED on `main` 2026-06-25** — `reflect-config.json`
> gained 5 entries (318→323: `NativePRNG`, `SecurityUtils`, `ForwardedRequestCustomizer$Forwarded`,
> `org.restlet…ServerAdapter`, a Xerces factory) and a minimal fixture `serve-rest-mock.ikanos.yaml`
> exposing `GET /greet` → real native `200` (was hanging). **The blocker is lifted**: a *real-HTTP*
> native e2e of an HTTP-adapter feature (REST, Skill, Control) is now viable. A first native
> control-port smoke — a real `GET /health/live` on the control port `9619` already declared in
> `serve-http.ikanos.yaml` — is specified in
> [`handoff-control-port-native-smoke.md`](./handoff-control-port-native-smoke.md) (the step lives in
> the nightly `test-binary` job; agents may not edit `.github/workflows/`, so it is handed off). The
> earlier `ControlPortNativeIT` scaffold (parked on `origin/chore/issue-578-control-port-native-it`,
> `a4fff4b3`) used the `-Dikanos.native.bin` opt-in vector; the nightly-smoke step is the lighter,
...
- **Control port** (#14): launch a `type: control` capability; HTTP GET `/health/live` (the reliable always-200 probe), then `/health/ready` and `/status`. `/metrics` + `/traces` return 503 unless OTel is active, so assert their reachability, not a 200. No backend needed — a good first **NATIVE** candidate, now **unblocked** since #594/#596 merged. The real-HTTP smoke step (`GET /health/live` on the native binary's control port `9619`) is specified in [`handoff-control-port-native-smoke.md`](./handoff-control-port-native-smoke.md) (issue #608); the earlier `-Dikanos.native.bin` opt-in `ControlPortNativeIT` scaffold (parked on `origin/chore/issue-578-control-port-native-it`) is the heavier alternative.
...
| 🔴 High | A1 | Control port endpoints — real-HTTP `GET /health/live` on the native binary (control port `9619` already in the fixture); step handed off ([`handoff-control-port-native-smoke.md`](./handoff-control-port-native-smoke.md), #608), unblocked since #594/#596 | Low — no backend |
...
| 🟠 Medium | A0 | **Extend the existing native smoke** (#581 `serve-http` fixture) to drive the **MCP** and **Skill** adapters at native level — both ports are already declared in the fixture but never probed. Now **unblocked** since #594/PR #596 merged (a *real* HTTP call replaces today's TCP-probe); follow the pattern in [`handoff-control-port-native-smoke.md`](./handoff-control-port-native-smoke.md) | Medium — fixture exists, add a real client call |
