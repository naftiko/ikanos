...
public class ControlPortMixin {
    ...
    String resolveAddress() {
        ...
        String env = System.getenv("NAFTIKO_CONTROL_ADDRESS");
    ...
    int resolvePort() {
        ...
        String env = System.getenv("NAFTIKO_CONTROL_PORT");
        if (env != null && !env.isBlank()) {
            try {
    ...
    int discoverPortFromYaml() {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(cwd, "*.{yml,yaml}")) {
            ...
            for (Path path : stream) {
                try {
                    NaftikoSpec spec = mapper.readValue(path.toFile(), NaftikoSpec.class);
