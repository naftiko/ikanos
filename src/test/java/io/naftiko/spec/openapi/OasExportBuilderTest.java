...
public class OasExportBuilderTest {
    ...
    void buildShouldMapInfoLabelToTitle() {
        NaftikoSpec spec = minimalSpec("My API", "A test API");
    ...
    void buildShouldUseDefaultTitleWhenNoLabel() {
        NaftikoSpec spec = minimalSpec(null, null);
        OasExportResult result = builder.build(spec, null);

        assertEquals("Naftiko Capability", result.getOpenApi().getInfo().getTitle());
    ...
    void buildWithV31ShouldProduceReparsableSpec() {
        NaftikoSpec spec = minimalSpec("Test 31", "OAS 3.1 test");
        RestServerSpec rest = getRestServer(spec);
    ...
    private NaftikoSpec minimalSpec(String label, String description) {
        NaftikoSpec spec = new NaftikoSpec();
    ...
    private RestServerSpec getRestServer(NaftikoSpec spec) {
